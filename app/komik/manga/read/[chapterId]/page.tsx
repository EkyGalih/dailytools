...
export default async function MangaReadPage({ params }: { params: Promise<{ chapterId: string }> }) {
    ...
    const detail = mangaEndpoint
    ? await getKomikDetail(mangaEndpoint)
    : null
        ...
        <main className="min-h-screen bg-[#050505] text-zinc-100 selection:bg-orange-500/30">
            ...
            <header className="sticky top-0 z-[60] bg-[#050505]/60 backdrop-blur-2xl border-b border-white/5 px-6 py-3 flex items-center justify-between transition-all duration-500">
                <div className="flex items-center gap-4">
                    <Link href={`/komik/manga/${mangaEndpoint}`} className="group flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-zinc-400 hover:text-white transition-all">
                        ...
                        <div className="p-12 bg-zinc-950 border-t border-white/5 flex flex-col items-center gap-8">
                            ...
                            <div className="flex items-center gap-4 w-full max-w-md">
                                {chapterData.prev_chapter ? (
                                    <Link href={`/komik/manga/read/${chapterData.prev_chapter}`} className="flex-1 text-center py-4 rounded-2xl bg-zinc-900 border border-zinc-800 text-[10px] font-black uppercase tracking-widest hover:border-orange-500/50 hover:bg-zinc-800 transition-all active:scale-95">
                                        ← Prev
                                    </Link>
                                ...
                                {chapterData.next_chapter ? (
                                    <Link href={`/komik/manga/read/${chapterData.next_chapter}`} className="flex-2 px-10 py-4 rounded-2xl bg-gradient-to-r from-orange-600 to-amber-500 text-white text-[10px] font-black uppercase tracking-widest hover:scale-105 hover:shadow-[0_10px_30px_rgba(234,88,12,0.3)] transition-all active:scale-95 text-center">
                                        Next Chapter →
                                    </Link>
                                ) : (
                                    <Link href={`/komik/manga/${detail?.data?.endpoint}`} className="flex-1 py-4 rounded-2xl bg-zinc-900 border border-zinc-800 text-[10px] font-black uppercase tracking-widest text-center">Done</Link>
                                )}
                            </div>
                        </div>
                        {images.map((img, i) => (
                        ...
                        <div className="p-12 bg-zinc-950 border-t border-white/5 flex flex-col items-center gap-8">
                            ...
                            <div className="flex items-center gap-4 w-full max-w-md">
                                ...
                                {chapterData.next_chapter ? (
                                    ...
                                    <Link href={`/komik/manga/${detail?.endpoint}`} className="flex-1 py-4 rounded-2xl bg-zinc-900 border border-zinc-800 text-[10px] font-black uppercase tracking-widest text-center">Done</Link>
                                )}
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="p-6 bg-zinc-900/30 border border-white/5 rounded-[2.5rem] backdrop-blur-xl flex flex-col h-full overflow-hidden">
                            ...
                            <div className="flex-grow overflow-y-auto pr-2 space-y-2 no-scrollbar">
                                {chapters.map((ch: any) => {
                                    ...
                                    return (
                                        <Link key={ch.endpoint} href={`/komik/manga/read/${ch.endpoint}`} className={`group relative flex items-center justify-between px-5 py-3.5 rounded-2xl border transition-all duration-300 ${active ? "bg-orange-600 border-orange-500 text-white" : "bg-white/[0.02] border-white/5 text-zinc-500 hover:text-zinc-200"}`}>
                                            <span className="text-[11px] font-black uppercase tracking-tighter">{ch.title || `Ch. ${ch.chapter_number}`}</span>
