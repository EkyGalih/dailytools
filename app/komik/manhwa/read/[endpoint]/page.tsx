...
export async function generateMetadata({ params }: { params: Promise<{ endpoint: string }> }): Promise<Metadata> {
    ...
    const title = `Reading Mode: Baca Manhwa Chapter ${endpoint} Sub Indo | Tamanto`
    ...
    return {
        ...
        alternates: {
            ...
            canonical: `${site}/komik/manhwa/read/${endpoint}`,
        },

        openGraph: {
            title: title,
            description: description,
            url: `${site}/komik/manhwa/read/${endpoint}`,
            siteName: 'Tamanto',
            type: 'article',
            images: [
                {
...
export default async function MangaReadPage({ params }: { params: Promise<{ endpoint: string }> }) {
    const { endpoint } = await params
    const chapterData = await getKomikImages(endpoint)
    const manhwaEndpoint =
        chapterData.manga_endpoint?.includes("manga/")
            ? chapterData.manga_endpoint.replace("manga/", "")
            : null

    const detail = manhwaEndpoint
        ? await getKomikDetail(manhwaEndpoint)
        : null
        ...
        <main className="min-h-screen bg-[#070708] text-zinc-100 selection:bg-cyan-500/30">
            ...
            <header className="sticky top-0 z-[60] bg-[#070708]/80 backdrop-blur-2xl border-b border-white/[0.05] px-4 md:px-10 py-4 flex items-center justify-between transition-all duration-500">
                <div className="flex items-center gap-6">
                    <Link href={`/komik/manhwa/${manhwaEndpoint}`} className="group flex items-center gap-2.5 px-4 py-2 rounded-full bg-white/[0.03] border border-white/5 text-[10px] font-black uppercase tracking-widest text-zinc-400 hover:text-cyan-400 hover:border-cyan-500/30 transition-all">
                    ...
                    <div className="flex flex-col">
                        ...
                        <p className="text-[8px] font-bold text-zinc-500 uppercase tracking-widest truncate max-w-[150px] md:max-w-xs">
                            {chapterData.title || " Mode Membaca"}
                        </p>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-4">
                    <div className="hidden lg:flex items-center gap-2 px-4 py-2 rounded-full bg-zinc-900/50 border border-white/5">
                        ...
                        <span className="text-[9px] font-black text-zinc-400 uppercase tracking-[0.2em]">Tamanto System v2</span>
                    </div>
                </div>
            </header>
                    ...
                    <div id="reader-container" className="relative space-y-0 shadow-[0_0_100px_rgba(0,0,0,0.8)] rounded-3xl overflow-hidden border border-white/[0.05] bg-[#0d0d0e] max-w-[900px] mx-auto w-full transition-all duration-700">
                        ...
                        <div className="p-10 bg-zinc-950/50 flex flex-col items-center gap-6 border-b border-white/5">
                            ...
                            <div className="flex items-center gap-3 w-full max-w-lg">
                                {chapterData.prev_chapter ? (
                                    <Link href={`/komik/manhwa/read/${chapterData.prev_chapter}`} className="flex-1 py-4 rounded-2xl bg-zinc-900 border border-white/5 text-[10px] font-black uppercase tracking-widest text-zinc-400 hover:text-white hover:border-cyan-500/50 transition-all text-center">
                                        ← Prev
                                    </Link>
                                ...
                                {chapterData.next_chapter ? (
                                    <Link href={`/komik/manhwa/read/${chapterData.next_chapter}`} className="flex-[2] py-4 rounded-2xl bg-gradient-to-r from-cyan-600 to-violet-600 text-white text-[10px] font-black uppercase tracking-widest hover:shadow-[0_0_30px_rgba(6,182,212,0.3)] hover:scale-[1.02] transition-all text-center">
                                        Next Chapter →
                                    </Link>
                                ) : (
                                    <Link href={`/komik/manhwa/${manhwaEndpoint}`} className="flex-1 py-4 rounded-2xl bg-zinc-900 border border-white/5 text-[10px] font-black uppercase tracking-widest text-center">Finished</Link>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="p-16 bg-zinc-950/50 border-t border-white/5 flex flex-col items-center gap-8">
                            ...
                            <div className="flex items-center gap-4 w-full max-w-lg">
                                {chapterData.prev_chapter && (
                                    <Link href={`/komik/manhwa/read/${chapterData.prev_chapter}`} className="flex-1 py-4 rounded-2xl bg-zinc-900 border border-white/5 text-[10px] font-black uppercase tracking-widest text-zinc-400 hover:text-white transition-all text-center">
                                        ← Prev
                                    </Link>
                                )}
                                <Link href={`/komik/manhwa/${manhwaEndpoint}`} className="flex-1 py-4 rounded-2xl bg-zinc-900 border border-white/5 text-[10px] font-black uppercase tracking-widest text-zinc-400 hover:text-cyan-400 transition-all text-center">
                                    Daftar Chapter
                                </Link>
                                {chapterData.next_chapter && (
                                    <Link href={`/komik/manhwa/read/${chapterData.next_chapter}`} className="flex-[2] py-4 rounded-2xl bg-gradient-to-r from-cyan-600 to-violet-600 text-white text-[10px] font-black uppercase tracking-widest hover:shadow-[0_0_40px_rgba(124,58,237,0.3)] transition-all text-center">
                                        Next Chapter →
                                    </Link>
                                )}
                            </div>
                    ...
                    <aside className="hidden lg:block sticky top-28 h-[calc(100vh-140px)] transition-all duration-500">
                        <div className="p-6 bg-zinc-900/40 border border-white/5 rounded-[2.5rem] backdrop-blur-2xl flex flex-col h-full shadow-2xl">
                            ...
                            <div className="flex-grow overflow-y-auto pr-2 space-y-2 custom-scrollbar">
                                {chapters.map((ch: any) => {
                                    const active = ch.endpoint === endpoint
                                    return (
                                        <Link
                                            key={ch.endpoint}
