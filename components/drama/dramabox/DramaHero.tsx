...
export default function DramaHero({ activeChannel }: { activeChannel?: DramaChannelSlug }) {
    ...
    return (
        <header className="relative overflow-hidden bg-[#050505] pt-12 pb-24 md:pt-28 md:pb-40">
            {/* Mesh Gradient Background */}
            <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full h-full opacity-30">
                <div className="absolute top-[-20%] left-[-10%] w-[70%] h-[70%] bg-indigo-900/40 blur-[80px] md:blur-[120px] rounded-full" />
                <div className="absolute bottom-[-10%] right-[-10%] w-[60%] h-[60%] bg-purple-900/30 blur-[80px] md:blur-[120px] rounded-full" />
            </div>
            ...
            <div className="relative max-w-7xl mx-auto px-4 md:px-6">
                <div className="flex flex-col items-start gap-4 md:gap-6">
                    ...
                    <h1 className="flex flex-wrap items-center gap-3 md:gap-4 text-3xl md:text-6xl font-black text-white italic tracking-tighter leading-tight md:leading-none uppercase">
                        <span className="relative h-10 w-10 md:h-14 md:w-14 bg-white/10 p-1.5 md:p-2 rounded-xl md:rounded-2xl border border-white/10 backdrop-blur-sm">
                            <Image src={channel.icon} alt="" fill className="object-contain p-1.5" priority />
                        </span>
                        <span>
                    ...
                    <p className="max-w-2xl text-zinc-400 text-sm md:text-lg font-medium leading-relaxed">
                        {channel.description} <span className="hidden md:inline">Nikmati ribuan episode drama China viral dalam format vertikal yang cocok untuk waktu luang Anda.</span>
                    </p>

                    {/* CHANNEL NAV (Optimized for Mobile) */}
                ...
                {/* Share Icons - Tetap tersembunyi di mobile untuk menjaga kebersihan visual */}
