    ...
    return (
        <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-[100] flex items-center gap-3 p-2.5 bg-[#09090b]/80 backdrop-blur-3xl border border-white/5 rounded-full shadow-[0_25px_50px_rgba(0,0,0,0.8)]">
            ...
            <div className="flex flex-col gap-1.5 px-3 border-r border-white/10 mr-1">
                <div className={`w-1 h-1 rounded-full transition-all duration-500 ${viewMode === 'normal' ? 'bg-orange-500 shadow-[0_0_8px_rgba(249,115,22,0.8)] scale-150' : 'bg-zinc-800'}`} />
                <div className={`w-1 h-1 rounded-full transition-all duration-500 ${viewMode === 'focus' ? 'bg-orange-500 shadow-[0_0_8px_rgba(249,115,22,0.8)] scale-150' : 'bg-zinc-800'}`} />
                <div className={`w-1 h-1 rounded-full transition-all duration-500 ${viewMode === 'ultra' ? 'bg-orange-500 shadow-[0_0_8px_rgba(249,115,22,0.8)] scale-150' : 'bg-zinc-800'}`} />
            </div>

            {/* Action Button */}
            <button
                onClick={cycleMode}
                className="group relative flex items-center gap-4 px-8 py-3.5 rounded-full bg-orange-600 hover:bg-orange-500 text-black transition-all duration-500 shadow-[0_10px_30px_rgba(234,88,12,0.4)] overflow-hidden"
            >
            ...
            {viewMode !== 'normal' && (
                <button
                    ...
                    className="p-3.5 rounded-full bg-zinc-900 border border-white/5 text-zinc-400 hover:text-orange-500 hover:bg-zinc-800 transition-all shadow-xl"
                >
                    <Maximize2 size={18} />
                </button>
            )}
